...
-- 3. Create user_subscriptions if not exists
...
-- 4. Create payment_transactions if not exists
...
-- 5. Create payment_gateways if not exists
...
-- 6. Create service_providers if not exists
...
-- 7. Create user_flags if not exists
...
-- 8. Add conversation_id to security_logs if missing
DO $$
BEGIN
...
-- 9. Add missing columns to conversations
DO $$
BEGIN
...
-- 11. Insert default payment gateways if table is empty
DO $$
BEGIN
...
SELECT 'Tables fixed! All missing columns and tables have been created.' as status;
