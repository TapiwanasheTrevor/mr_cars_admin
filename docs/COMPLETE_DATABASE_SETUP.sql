-- =============================================
-- COMPLETE DATABASE SETUP FOR MR CARS ADMIN
-- =============================================
...
-- Additional: User Flags for evasion detection
...
CREATE POLICY "Admins can view all user flags"
  ON user_flags FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role IN ('super_admin', 'moderator')));
...
BEFORE UPDATE ON user_flags FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();
