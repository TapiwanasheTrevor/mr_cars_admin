...
-- 3. Create payment_gateways FIRST
...
-- 9. Create/Update notifications table with correct schema for admin dashboard
DO $$
BEGIN
  -- Drop old notifications table if it exists with wrong schema
  IF EXISTS (
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type TEXT NOT NULL CHECK (type IN ('inquiry', 'order', 'appointment', 'user', 'system')),
  read BOOLEAN DEFAULT false,
  data JSONB DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- 10. Add conversation_id to security_logs if missing
DO $$
BEGIN
...
-- 11. Add missing columns to conversations
DO $$
BEGIN
...
-- 12. Create indexes ONLY if columns exist
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_subscriptions' AND column_name='plan_id') THEN
    CREATE INDEX IF NOT EXISTS idx_user_subscriptions_plan ON user_subscriptions(plan_id);
  END IF;

  -- Payment transactions indexes
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='payment_transactions' AND column_name='user_id') THEN
    CREATE INDEX IF NOT EXISTS idx_payment_transactions_user ON payment_transactions(user_id);
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='payment_transactions' AND column_name='gateway_id') THEN
    CREATE INDEX IF NOT EXISTS idx_payment_transactions_gateway ON payment_transactions(gateway_id);
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='payment_transactions' AND column_name='status') THEN
    CREATE INDEX IF NOT EXISTS idx_payment_transactions_status ON payment_transactions(status);
  END IF;

  -- Other indexes
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='user_flags' AND column_name='user_id') THEN
    CREATE INDEX IF NOT EXISTS idx_user_flags_user ON user_flags(user_id);
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='security_logs' AND column_name='conversation_id') THEN
...
-- 13. Insert default payment gateways if table is empty
DO $$
BEGIN
...
-- =============================================
SELECT
  'Admin dashboard setup complete! All tables created successfully.' as status,
  ...
  (SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_subscriptions')) as subscriptions_exists,
  (SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'service_providers')) as service_providers_exists;
